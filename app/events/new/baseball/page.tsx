...
// Baseball Game Form with API Search and Player Stats
...
import { MatchSearch, type MatchResult, type BaseballGameResult, type BaseballPlayerAppearance, type PlayerAppearance } from '@/components/shared/MatchSearch';
import { PlayerStatsEditor, type BaseballPlayer, type Player } from '@/components/events/PlayerStatsEditor';
import { TeamBadge } from '@/components/shared/TeamBadge';

const baseballSchema = z.object({
...
type BaseballFormData = z.infer<typeof baseballSchema>;

export default function BaseballFormPage() {
  ...
  const {
    ...
    resolver: zodResolver(baseballSchema),
    defaultValues: {
      date: new Date().toISOString().split('T')[0],
      homeScore: 0,
      awayScore: 0,
      venueCountry: 'USA', // Default for MLB
    },
  });
  ...
  const handleMatchSelect = (result: MatchResult) => {
    const match = result as BaseballGameResult;
    ...
    setValue('venueName', (match as BaseballGameResult).venue || '');
  ...
  const onSubmit = async (data: BaseballFormData) => {
    ...
    try {
      ...
      const response = await fetch('/api/events/baseball', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen pb-20">
      <Header title="Log Baseball Game" />
      ...
      <main className="px-4 py-6 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-5 gap-4 items-end">
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="text-2xl font-mono font-bold w-16 text-center">
                    {homeScore} - {awayScore}
                  </span>
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <button
                    ...
                    className="w-full py-2 px-4 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors"
                  >
                    + Home Run
                  </button>
                </div>
          ...
          {/* Competition */}
          <div>
            <label className="block text-sm font-medium mb-2">League (Optional)</label>
            <Input {...register('competition')} placeholder="e.g., MLB, Minor League" />
          </div>

          {/* Venue */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2">Ballpark</label>
          ...
          <PlayerStatsEditor
            ...
            players={players}
            onPlayersChange={(players) => setPlayers(players as BaseballPlayer[])}
          />
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Big plays, home runs, memorable moments..."
            />
          </div>
